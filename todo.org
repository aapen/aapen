...
7. initialize host
   ...
   3. if ~core_hardware_config_2 hs_phy_type \== phy_ulpi *and* fs_phy_type \== phy_type_dedicated~ *and* ~core_usb_config~ has bit ~cfg_ulpi_fsls~ set
      1. set bit ~fsls_pclk_sel_48_mhz~ in ~host_config~
   4. else
      1. set bit ~fsls_pclk_sel_30_60_mhz~ in ~host_config~
   5. flush tx fifos
   6. flush rx fifo
   7. if bit ~host_port_power~ is not set in ~host_port~
