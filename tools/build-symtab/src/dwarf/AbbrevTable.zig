...
// TODO consider using a map of u64 -> Decl
decls: std.ArrayListUnmanaged(Decl) = .{},
loc: Loc,

pub fn deinit(table: *AbbrevTable, allocator: Allocator) void {
...
pub const Decl = struct {
    ...
    pub fn deinit(decl: *Decl, allocator: Allocator) void {
        decl.attrs.deinit(allocator);
    }
...
pub const Attr = struct {
    ...
    pub fn getString(attr: Attr, value: []const u8, dwf: Parser.Format, elf: Elf) ?[]const u8 {
        switch (attr.form) {
            dwarf.FORM.string => {
            ...
            dwarf.FORM.strp => {
                ...
                return elf.getDwarfString(off);
            },
            else => return null,
        }
    }

    pub fn getSecOffset(attr: Attr, value: []const u8, dwf: Parser.Format) ?u64 {
        return switch (attr.form) {
    ...
    pub fn getReference(attr: Attr, value: []const u8, dwf: Parser.Format) !?u64 {
        var stream = std.io.fixedBufferStream(value);
        const reader = stream.reader();
        return switch (attr.form) {
