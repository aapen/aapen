...
pub fn connect(self: *Self, id: IrqId, handler: IrqHandlerFn) void {
...
fn handle(self: *Self, id: IrqId) void {
    self.handlers[id.index](self, id);
}

fn basicIrqHandleIfRaised(self: *Self, pending: u32, check: u32, irq_id: IrqId) void {
    if ((pending & check) != 0) {
        self.handle(irq_id);
    }
}
