...
fn parseHubConfiguration(allocator: Allocator) !void {
    ...
    const if0 = dev.interfaces[0] orelse return;
    expectEqual(@src(), @as(u8, 1), if0.endpoint_count);

    expect(@src(), dev.endpoints[0][0] != null);
    const ep0 = dev.endpoints[0][0] orelse return;
    expectEqual(@src(), @as(u8, 0x81), ep0.endpoint_address);
    expect(@src(), ep0.isType(TransferType.interrupt));
}
